...
function createTask(grunt) {
    return function (taskName, targetName) {
        ...
        try {
            try {
                var store = grunt.file.readJSON(stamp);
                oldCounter = store.fileNumber;
            } catch (err) {
                grunt.log.writeln('Override cached data');
            }
            var fil = fs.statSync(stamp);
            previous = fil.mtime > fil.ctime ? fil.mtime : fil.ctime;
        ...
        util.filterFilesByTime(files, previous, override, oldCounter, function (e, newerFiles) {
            if (e) {
                return done(e);
            } else if (newerFiles.length === 0 && parseInt(oldCounter) === util.getCounter()) {
            ...
            var tasks = [
        qualified + (args ? ':' + args : ''),
        'newer-postrun:' + qualified + ':' + id + ':' + util.getCounter() + ':' + options.cache
      ];
...
module.exports = function (grunt) {
    ...
    grunt.registerTask(
        'newer-postrun', internal,
        function (taskName, targetName, id, count, dir) {

            // if dir includes a ':', grunt will split it among multiple args
            dir = Array.prototype.slice.call(arguments, 4).join(':');
    ...
    grunt.registerTask(
        'newer-clean', clean, function (taskName, targetName) {
             ...
             * custom cache dir is provided, it is up to the user to keep it
             * clean.
             */
