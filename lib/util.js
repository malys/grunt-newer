...
var filterPathsByTime = exports.filterPathsByTime = function (paths, time,
    ...
    async.map(paths, fs.stat, function (err, stats) {
        ...
        var newerPaths = paths.filter(function (filePath, index) {
            var staI = stats[index];
            var ref = staI.mtime > staI.ctime ? staI.mtime : staI.ctime;
            var newer = ref > time;
            if (!newer) {
...
var anyNewer = exports.anyNewer = function (paths, time, override, callback) {
    ...
    function iterate() {
        fs.stat(paths[complete], function (err, stats) {
            ...
            var ref = stats.mtime > stats.ctime ? stats.mtime : stats.ctime;
            if (ref > time) {
    ...
    async.map(files, function (obj, done) {
        if (obj.dest) {
            fs.stat(obj.dest, function (err, stats) {
                ...
                var ref = stats.mtime > stats.ctime ? stats.mtime : stats.ctime;
                var result = anyNewer(obj.src, ref, override, function (err, any) {
                    done(err, any && obj);
                });
...
/**
 * Return Watched file number
 * @return {int}  Watched file number
 */
var getCounter = exports.getCounter = function () {
    return counter;
};